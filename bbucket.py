...
config = bb.Config()
config.load(bb.BUCKETCONFIG)
account = bb.Account(config.accounts, None)
credential = bb.Credentials(account.username, account.password)

configs = ['config/base.cfg','config/core.cfg']

@task()
def check(owner='nantic', config=None):
    Config = read_config_file(config)
    session = bb.Session(credential)
    manager = session.create_repo_manager(owner=owner)
    bb_repos = manager.get_repos()
    repos = set(Config.sections())
    brepos = set()
    for c in configs:
        cc = read_config_file(c)
        repos = repos - set(cc.sections())

    for brepo in bb_repos:
        if not 'trytond-' in brepo.name:
            continue
        bname = brepo.name.replace('trytond-','')
        brepos.add(bname)


    print t.bold("Repos in Config File but not in Bitbucket or Nor Owned")
    print ", ".join(repos-brepos)
    print t.bold("\nRepos in  Bitbucket but not in Config File")
    print ", ".join(brepos-repos)
